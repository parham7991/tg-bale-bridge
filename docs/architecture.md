...
| Log buffer | `bridge/logbuf/` (package) | In-memory ring buffer serving `/logs` and the dashboard `/api/logs` — since v2.17.0: `types_map` capacity/format/clamp · `ring` RingEngine (pure deque + clamped tail) · `handler` RingBufferHandler (never-raise emit, `.lines` alias) · `bootstrap` InstallEngine (root-logger attach) · `facade` legacy surface (`install` + `RingBufferHandler`) |
